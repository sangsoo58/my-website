# PHASE 3 — Expanded Edition Final Editorial TOC

Status: **EDITORIAL PROPOSAL / USER CONFIRMATION REQUIRED / PAGE NUMBERS PENDING**

## 편집 제안

Expanded Edition은 기존 7개 PART 체계를 유지하고, PART 4의 공기압축기 다음에 **보일러 시스템 운전최적화**를 신규 Chapter 15로 삽입하는 안을 1차 편집안으로 제안한다. 사용자 확인 전 최종 목차로 확정하지 않는다.

CASE ID는 Chapter 번호 변경과 무관하게 **CASE01~CASE82를 영구 유지**한다.

## PART 1. 데이터 기반 에너지절감 컨설팅의 기본
- Chapter 1. 에너지절감 컨설팅의 전체 프로세스
- Chapter 2. 에너지 데이터를 어떻게 이해할 것인가
- Chapter 3. 에너지성능 KPI와 Benchmark
- Chapter 4. Baseline 모델과 M&V
- Chapter 5. 운전최적화 시나리오 설계

## PART 2. 제조공정 에너지절감
- Chapter 6. 열처리공정 Baseline과 표준 운전
- Chapter 7. 주조·용해공정의 생산성과 에너지 원단위
- Chapter 8. Former·사출·압출 공정 데이터분석
- Chapter 9. 소결공정의 설비별 성능 비교

## PART 3. 반도체·디스플레이·전자부품 Utility 최적화
- Chapter 10. 반도체 Cleanroom HVAC
- Chapter 11. 냉동기 Plant 최적화
- Chapter 12. 반도체 Multi-Utility 통합 최적화
- Chapter 13. 전장품·전자부품 제조 Utility

### Phase 3 확장
- Ch.11: CASE53,55,56,57,58,60,62,63,64,78 추가
- Ch.12: CASE42,43,44,45 추가

## PART 4. 산업 Utility 및 에너지다소비 산업
- Chapter 14. 공기압축기 시스템 운전최적화
- **Chapter 15. 보일러 시스템 운전최적화 [NEW]**
- Chapter 16. 화학공장 통합 Utility
- Chapter 17. 철강산업 에너지최적화
- Chapter 18. 비철금속·2차전지 원료 산업
- Chapter 19. 상하수도 대형 펌프시스템
- Chapter 20. 발전소 보조기기

### Phase 3 확장
- Ch.14: CASE52,54,59,61 추가
- Ch.15: CASE65~69 신규
- Ch.20: CASE75,76 추가

## PART 5. 건물·데이터센터·지역난방
- Chapter 21. 기존 대형건물 에너지진단
- Chapter 22. BEMS와 Continuous Commissioning
- Chapter 23. 신축건물 Energy Modeling
- Chapter 24. 데이터센터 PUE와 냉방 최적화
- Chapter 25. 지역난방 운전최적화

### Phase 3 확장
- Ch.23: CASE48,49,51 추가 / CASE50 정본 확인 완료·별도 포함 여부 사용자 결정 대기
- Ch.24: CASE46,47,70~74,77,79~82 추가

## PART 6. 에너지 데이터 분석과 성과 검증
- Chapter 26. MES와 EMS 데이터를 연결하는 방법
- Chapter 27. 에너지 예측 모델 구축
- Chapter 28. 데이터 기반 이상 진단과 운전 추천
- Chapter 29. 에너지절감 M&V의 실제

## PART 7. 현장 적용 Toolkit와 활용법

## 편집 규칙
- 현재 CASE01~41의 CASE ID는 변경하지 않는다.
- Chapter 15 삽입안이 승인될 경우 기존 Chapter 15~28은 Expanded Edition에서 +1 이동한다.
- 페이지 번호는 사용자 승인 후 조판 단계에서 확정한다.
- Current 41-CASE Book Reader는 Expanded Edition 공개 전까지 기존 번호/페이지를 유지한다.
- Expanded Edition Web↔Book mapping은 최종 페이지 확정 후 별도 Edition으로 활성화한다.


## Baseline Protection

- 현재 Print Baseline 및 Editable Word Master는 변경하지 않는다.
- 현재 41 CASE Book Reader도 변경하지 않는다.
- 이 문서는 향후 책 개정 후보를 검토하기 위한 Draft다.
- CASE42~82의 Book 반영 여부·Chapter 배치·최종 제목은 사용자 확인 후 확정한다.
